...
### C) Bulk submit a CSV of products as a Groq Batch job (50% cheaper, requires paid tier)
...
> **Note:** Groq's Batch API (`client.files.create(purpose="batch")`) currently
> returns `403 Not available for your plan` on free-tier accounts. The code
> is correct and matches Groq's documented Batch API spec — it simply
> requires a paid/developer tier to execute.

### D) Bulk (free-tier fallback)  local batch simulation

Demonstrates the identical bulk-orchestration concept (CSV in → many
requests processed concurrently → validated JSONL out) without depending
on the paid Batch API. Routes every row through the real-time endpoint
using `asyncio.gather` + `Semaphore`, reusing the exact same
`prompt_engine` and Pydantic schema as everything else.

```bash
python main.py batch-simulate --input data/sample_products.csv --output outputs/simulated_results.jsonl
```

|               | `batch_pipeline.py` (Groq Batch API) | `batch_simulation.py` (local fallback) |
| ------------- | -------------------------------------- | ---------------------------------------- |
| Endpoint      | Async batch job                        | Real-time`chat.completions`            |
| Cost          | 50% cheaper                            | Full price                               |
| Turnaround    | 24h – 7 days                          | Seconds                                  |
| Tier required | Paid/developer                         | Any (free tier works)                    |
| Use case      | Overnight, thousands of rows           | Demo, testing, small bulk runs           |
